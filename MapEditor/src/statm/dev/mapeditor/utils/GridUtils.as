package statm.dev.mapeditor.utils
{
    ...
    public class GridUtils
    {
        ...
        {
            if (Math.abs(grid1.x - grid2.x) < 1 && Math.abs(grid1.y - grid2.y) < 1)
            {
                line.visible = false;
            }
            else
        ...
        public static function getPathBetween(grid1:Point, grid2:Point):Vector.<Point>
        {
            ...
            {
                var point:Point = new Point(grid1.x * GRID_WIDTH + GRID_WIDTH / 2 + dx * i / steps, grid1.y * GRID_HEIGHT + GRID_HEIGHT / 2 + dy * i / steps);
                var grid:Point = globalToGrid(point);

                if (!lastGrid || !grid.equals(lastGrid))
                {
                    result.push(grid);
                    lastGrid = grid;
                }
