package statm.dev.mapeditor.dom.layers
{
    ...
    public class Items extends DomNode
    {
        public function Items(root:DomNode)
        {
            ...
            _name = "物件";
            _parent = root;
            _children = new ArrayCollection([ _npcLayer = new NPCLayer(root),
                                              _mobLayerContainer = new MobLayerContainer(root),
                                              _transportPoints = new TransportPoints(root),
                                              _waypoints = new WaypointLayer(root),
                                              _mineralLayer = new MineralLayer(root),
                                              _markLayer = new MarkLayer(root),
                                              _routeLayerContainer = new RouteLayerContainer(root),
                                              _fxLayer = new FxLayer(root),
                                              _decorationLayer = new DecorationLayer(root)]);
            _npcLayer.parent =
                _mobLayerContainer.parent =
                _transportPoints.parent =
                _waypoints.parent =
                _mineralLayer.parent =
                _markLayer.parent =
                _routeLayerContainer.parent =
                _fxLayer.parent =
                _decorationLayer.parent = this;
        }
        ...
        private var _fxLayer:FxLayer;

        public function get fxLayer():FxLayer
        {
            return _fxLayer;
        }

        private var _decorationLayer:DecorationLayer;

        public function get decorationLayer():DecorationLayer
        {
        ...
        public function addItem(item:Item):void
        {
            ...
            {
                fxLayer.addItem(item);
            }
            else if (item is Decoration)
            {
                decorationLayer.addItem(item);
            }
        }
        ...
        override public function toXML():XML
        {
            ...
            results.appendChild(npcLayer.toXML())
                .appendChild(mobLayerContainer.toXML())
                .appendChild(transportPoints.toXML())
                .appendChild(waypoints.toXML())
                .appendChild(mineralLayer.toXML())
                .appendChild(markLayer.toXML())
                .appendChild(routeLayerContainer.toXML())
                .appendChild(fxLayer.toXML())
                .appendChild(decorationLayer.toXML());
        ...
        override public function readXML(xml:XML):void
        {
            ...
            xml.fxLayer[0] && this.fxLayer.readXML(xml.fxLayer[0]);
            xml.decorationLayer[0] && this.decorationLayer.readXML(xml.decorationLayer[0]);
        }
    }
}
