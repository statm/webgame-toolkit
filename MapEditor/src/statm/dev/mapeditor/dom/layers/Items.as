package statm.dev.mapeditor.dom.layers
{
    ...
    public class Items extends DomNode
    {
        public function Items(root:DomNode)
        {
            ...
            _name = "物件";
            _parent = root;
            _children = new ArrayCollection([ _npcLayer = new NPCLayer(root), _mobLayerContainer = new MobLayerContainer(root), _transportPoints = new TransportPoints(root), _waypoints = new WaypointLayer(root), _mineralLayer = new MineralLayer(root), _markLayer = new MarkLayer(root), _routeLayerContainer = new RouteLayerContainer(root), _fxLayer = new FxLayer(root)]);
            _npcLayer.parent = _mobLayerContainer.parent = _transportPoints.parent = _waypoints.parent = _mineralLayer.parent = _markLayer.parent = _routeLayerContainer.parent = _fxLayer.parent = this;
        }
        ...
        override public function toXML():XML
        {
            ...
            results.appendChild(npcLayer.toXML()).appendChild(mobLayerContainer.toXML()).appendChild(transportPoints.toXML()).appendChild(waypoints.toXML()).appendChild(mineralLayer.toXML()).appendChild(markLayer.toXML()).appendChild(routeLayerContainer.toXML()).appendChild(fxLayer.toXML());
