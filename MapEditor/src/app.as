...
override protected function mouseDownHandler(event:MouseEvent):void
{
    ...
    if (!(event.target is IFocusManagerComponent) && !(event.target is Label) && !(event.target is TextLine))
    {
        stage.focus = null;
    }
...
override protected function keyDownHandler(event:KeyboardEvent):void
{
    ...
    if (event.keyCode == Keyboard.SPACE && !(stage.focus is TextInput) && !(AppState.isDrawingMask() && isMouseDown))
    {
...
override protected function keyUpHandler(event:KeyboardEvent):void
{
    ...
    {
        if (!(stage.focus && (stage.focus is TextInput || stage.focus is TextLine || stage.focus is RichEditableText)))
        {
    ...
    else if (event.keyCode == Keyboard.MINUS && event.controlKey)
    {
        mapArea.scaleX -= .1;
        mapArea.scaleY -= .1;
    }
    else if (event.keyCode == Keyboard.EQUAL && event.controlKey)
    {
        mapArea.scaleX += .1;
        mapArea.scaleY += .1;
    }
    else if (event.keyCode == Keyboard.NUMBER_0 && event.controlKey)
    {
