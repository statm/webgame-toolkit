    ...
    public class SpritesheetWriter
    {
        ...
        public function writeActionSpritesheet(action:Action, path:File, isFX:Boolean = true):void
        {
            ...
            for each (var direction:String in directions)
            {
                ...
                if (!batch || batch.batchState == ImageBatchState.COMPLETE || batch.batchState == ImageBatchState.LOADING)
                {
                    unloadedBatchCount--;
                }
                else
                {
                    batch.addEventListener(ImageBatchEvent.COMPLETE, function(event:ImageBatchEvent):void
                    {
...
//				assembledSpritesheet.draw(tf, mtx);
            ...
            for (i = 0; i < l; i++)
            {
                if (originalImages[i])
                {
                    ...
                    var offsetY:int = (useVerticalOffset ? (originalSize.height / 2 - action.anchor) : 0);
                    ...
                    if (!cropBound.isEmpty())
                    {
                        ...
                        frameXML.appendChild(<reg-point>{Math.floor(originalSize.width / 2) - cropBound.x},{Math.floor(originalSize.height / 2) - cropBound.y + offsetY}</reg-point>).appendChild(<rect>{frameBound.x},{frameBound.y},{frameBound.width},{frameBound.height}</rect>);
