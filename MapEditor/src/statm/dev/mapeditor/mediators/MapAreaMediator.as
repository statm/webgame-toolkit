package statm.dev.mapeditor.mediators
{
	...
	public class MapAreaMediator extends Mediator
	{
		...
		public function MapAreaMediator(mediatorName : String = null, viewComponent : Object =
			null)
		{
		...
		override public function listNotificationInterests() : Array
		{
			return [AppNotificationCode.MAP_DATA_READY,
				...
				AppNotificationCode.VIEWPORT_CHANGED,
		...
		{
			AppState.setMapSize(new Point(event.currentTarget.sourceWidth, event.currentTarget.
				sourceHeight));
		}
		...
		private function mapScroller_resizeHandler(event : Event) : void
		{
			...
			if (!viewport)
			{
				AppState.setViewport(new Rectangle(0, 0, mapArea.mapScroller.width, mapArea.
					mapScroller.height));
			}
			else
			{
		...
		private function mapScroller_mouseDownHandler(event : MouseEvent) : void
		{
			...
			if ((!AppState.isMovingViewport()
				&& !AppState.isDrawingMask()
				&& !AppState.isMovingGrid()
				&& !AppState.isDrawingItem()
				&& !AppState.isMovingItem())
				|| !map)
			{
				return;
			}
		...
		private function stage_mouseMoveHandler(event : MouseEvent) : void
		{
			...
			{
				moveViewport(
					mapArea.mapScrollingGroup.horizontalScrollPosition
					- (event.stageX - dragAnchor.x) * 2,
					mapArea.mapScrollingGroup.verticalScrollPosition
					- (event.stageY - dragAnchor.y) * 2
					);
...
//					AppState.setCurrentSelection(newItem);
		...
		private function ensureObjOnScreen(obj : DomObject) : void
		{
			...
			var itemRect : Rectangle = new Rectangle(itemPos.x + obj.border.x,
				itemPos.y + obj.border.y,
				obj.border.width,
				obj.border.height);
