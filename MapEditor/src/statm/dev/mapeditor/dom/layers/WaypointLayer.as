        ...
        public function connectAdjacentWaypoints(waypoint:Waypoint, createPath:Boolean = true):void
        {
            ...
            if (createPath)
            {
                ...
                for each (wp in this.children)
                {
                    ...
                    if (GridUtils.distance(wp, waypoint) <= GridUtils.WAYPOINT_DIST_THRESHOLD && isPathAvailable(GridUtils.getPathBetween(new Point(wp.x, wp.y), new Point(waypoint.x, waypoint.y))))
                    {
